...
void rgb2yuv420p(uint8_t *destination, uint8_t *rgb, size_t width,
                 size_t height) {
...
void add_video_frame(uint8_t *frame) {
  ...
  av_image_fill_arrays((AVPicture *)video_frame->data, video_frame->linesize,
                       yuv_buffer, video_frame->format, video_frame->width,
...
void open_video(int w, int h, int fps, int br, int preset_idx, int codec_idx,
                int format_idx) {
  ...
  avio_ctx = avio_alloc_context(avio_ctx_buffer, avio_ctx_buffer_size, 1, &bd,
  ...
  const char *presets[] = {"ultrafast", "veryfast", "fast",
...
void free_buffer() { av_free(bd.buf); }
...
void add_audio_frame(float *left, float *right, int size) {
  if (size + audio_buffer_size > max_audio_buffer_size) {
    audio_buffer_left =
        realloc(audio_buffer_left, (size + audio_buffer_size) * 4);
    audio_buffer_right =
        realloc(audio_buffer_right, (size + audio_buffer_size) * 4);
  }
  ...
  while (audio_buffer_size > audio_frame->nb_samples) {
    ...
    ret = swr_convert(audio_swr_ctx, audio_frame->data, dst_nb_samples,
    ...
    // Shift array buffer
    memcpy(audio_buffer_left, audio_buffer_left + audio_frame->nb_samples,
           audio_buffer_size * sizeof(float));
    memcpy(audio_buffer_right, audio_buffer_right + audio_frame->nb_samples,
           audio_buffer_size * sizeof(float));
